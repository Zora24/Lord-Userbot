...
ALIVE_LOGO = os.environ.get(
    "ALIVE_LOGO") or "https://telegra.ph/file/dc8f166c3e8f823a62e90.jpg"
...
async def check_alive():
    await bot.send_message(BOTLOG_CHATID, "```『TIDAK DIKETAHUI TELAH AKTIF』```")
    return
    ...
    try:
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query.startswith("@UserButt"):
                ...
                result = builder.article(
                    "Bantuan Lord ✗ tidak diketahui",
                    text="Daftar Modul",
                    buttons=[],
                    link_preview=True)
            else:
                result = builder.article(
                    "**Lord ✗ tidak diketahui**",
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                reply_pop_up_alert = f"Jangan Menggunakan Milik Lord {ALIVE_NAME} ツ"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
