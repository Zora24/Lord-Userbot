...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf tidak diketahui {ALIVE_NAME} Sedang AFK!**",
    f"**Maaf tidak diketahui {ALIVE_NAME} Sedang AFK\n Tunggu Sampai Dia Kembali Online!**",
    f"**Tidak diketahui {ALIVE_NAME} Sedang AFK\n Tunggulah Sampai Online**",
    f"**Maaf tidak diketahui {ALIVE_NAME} Sedang AFK!**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**⚠️ AFK!**\n**Tidak diketahui Telah AFK**\
        \n☛ **Alasan:** `{string}`")
    else:
        await afk_e.edit("**⚠️ AFK!**\n**Tidak diketahui Telah AFK**")
    if user.last_name:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**⚠️Tidak Diketahui {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☛ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**⚠️ Tidak Diketahui Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"⚠️ **Tidak Diketahui Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
