...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf Bos {ALIVE_NAME} Sedang AFK!**",
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**Halo Sayang Aku Kembali XiXiXi!**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**✘ Bos {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☞ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**☒ Bos Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☞ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"☒ **Bos Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☞ **Alasan**: `{AFKREASON}`")
                else:
