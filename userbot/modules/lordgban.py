...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Bos Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini ヅ`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh bosヅ`")
    ...
    if user:
        if user.id == 1353102497:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke Bos Apis, Dia Adalah Pembuat Saya ヅ`"
            )
        try:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Bos`")
    try:
...
async def gunben(userbot):
    ...
    try:
        ...
        return await dark.edit("`Mampus ada kesalahan ヅ`")
    if user:
        if user.id == 1353102497:
            return await dark.edit("**Bos Pengguna Tidak Bisa Terkena Perintah Ini, Karna Dia Pembuatku ヅ**")
        try:
        ...
        await dark.edit("`Harap Balas Ke Pesan Pengguna Bosヅ`")
    try:
